...
func Test_UserListProxy(t *testing.T) {
	...
	t.Run("FindUser - Empty cache", func(t *testing.T) {
		...
		if proxy.DidLastSearchUsedCache {
			t.Error("No user can be retruned from an empty cache")
		}
	})
	...
	t.Run("FindUser - One user, ask for the same user", func(t *testing.T) {
		...
		if !proxy.DidLastSearchUsedCache {
			t.Error("The user should have been return from the cache")
		}
	})

	t.Run("FindUser - overflowintg the stack", func(t *testing.T) {
