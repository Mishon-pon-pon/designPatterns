...
func TestPizzaDecorator_AddIngredient(t *testing.T) {
	...
	if !strings.Contains(pizzaResult, expectedText) {
		t.Errorf("When calling the add ingredient of the pizza decorator it must return the text %s the expected text, not '%s'",
			pizzaResult,
			expectedText,
		)
	}
...
func TestOnion_AddIngredient(t *testing.T) {
	...
	if err == nil {
		t.Error("When calling AddIngredient on the meat decorator without"+
			"and Ingredient in its Ingredient field must return an error"+
	...
	if !strings.Contains(onionResult, "onion") {
		t.Errorf("When calling the add ingredient of the onion decorator it"+
			"must retrun a text with the word 'onion', not '%s'", onionResult)
	}
}
...
func TestPizzaDecorator_FullStack(t *testing.T) {
	...
	if !strings.Contains(pizzaResult, expectedText) {
		t.Errorf("When asking for a pizza with onion and meat the retruned"+
